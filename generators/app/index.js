...
module.exports = class extends Generator {
  constructor(args, opts) {
    ...
    this.argument("appname", { type: String, required: true, description: 'The Application Name' });

    this.argument("-t", { type: String, required: false, description: 'testing' });
  }

  prompting() {
    // Have Yeoman greet the user.
    this.log(
      yosay(`${chalk.green(' ><(((º>')} \n Welcome to the badass ${chalk.red('B.A.S.S.')} generator! \n ${chalk.green(' ><(((º>')} `)
    );
    ...
    const prompts = [
      ...
      },
      {
        type: "input",
        name: 'prodUserName',
        message: `${chalk.red('Production')} Username (for "deploy-prod" script)`
      },
      {
        type: "confirm",
        name: 'useTsForce',
  ...
  writing() {
    ...
    this.fs.copyTpl(
      ...
      this.destinationPath(path.join(targetPages, `${templateProps.vfPageName}.page`)),
      templateProps
    );

    this.fs.copyTpl(
      this.templatePath(path.join(sourcePages, 'App.page-meta.xml')),
      this.destinationPath(path.join(targetPages, `${templateProps.vfPageName}.page-meta.xml`)),
      templateProps
    );

    this.fs.copyTpl(
      this.templatePath(path.join(dynamic, 'force-app', sfdxDefault, 'staticresources', '.gitkeep')),
      this.destinationPath(path.join(templateProps.sfdxProjectRoot, sfdxDefault, 'staticresources', '.gitkeep')),
      templateProps
    );
